...
const itemVariants = cva(
	"group/item flex w-full flex-wrap items-center squircle border text-sm transition-colors duration-100 outline-none focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 [a]:transition-colors [a]:hover:bg-muted",
	{
		variants: {
			variant: {
	...
	"flex shrink-0 items-center justify-center gap-2 group-has-data-[slot=item-description]/item:translate-y-0.5 group-has-data-[slot=item-description]/item:self-start [&_svg]:pointer-events-none",
	{
		variants: {
			variant: {
				...
				image:
					"size-10 overflow-hidden squircle group-data-[size=sm]/item:size-8 group-data-[size=xs]/item:size-6 [&_img]:size-full [&_img]:object-cover",
			},
		},
		defaultVariants: {
			variant: "default",
...
function ItemTitle({ className, ...props }: React.ComponentProps<"div">) {
	return (
		<div
			...
			className={cn("line-clamp-1 flex w-fit items-center gap-2 text-sm font-medium underline-offset-4", className)}
			{...props}
		/>
	);
}
...
export { Item, ItemMedia, ItemContent, ItemActions, ItemTitle, ItemDescription, ItemHeader, ItemFooter };
