...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
	return (
		<tr
			data-slot="table-row"
			className={cn("divide-x divide-border border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted", className)}
			{...props}
		/>
	);
}
...
function TableHead({ className, ...props }: React.ComponentProps<"th">) {
	return (
		<th
			...
			className={cn(
				"h-10 bg-muted px-2 text-center align-middle font-medium whitespace-nowrap text-foreground first:text-start",
				className,
			)}
			{...props}
		/>
