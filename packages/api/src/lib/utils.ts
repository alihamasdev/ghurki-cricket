...
export function calcBattingAverage(data: { runs: number | null; innings: number | null; notOuts: number | null }) {
...
export function calcStrikeRate(data: { runs: number | null; balls: number | null }) {
	const runs = data.runs ?? 0;
...
export function calcBowlingAverage(data: { runs: number | null; wickets: number | null }) {
	const runs = data.runs ?? 0;
...
export function calcBowlingEconomy(data: { runs: number | null; balls: number | null }) {
	const runs = data.runs ?? 0;
	const balls = data.balls ?? 0;
